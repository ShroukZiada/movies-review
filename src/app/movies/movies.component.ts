...
export class MoviesComponent implements OnInit, OnDestroy {
  ...
  getMovies(pageNum: number): void {
    this.moviesSubscrip = this._MoviesService
      .getMovieByPagination('movie', pageNum)
      .subscribe({
        next: (res) => {
          ...
          if (this.totalPages >= 11 && this.totalPages >= this.activatePageNum) {
            console.log('true', this.totalPages, this.activatePageNum);
        ...
        complete:()=>{
          this._MoviesService.shardPageNumberMovie.next(this.activatePageNum);
        },
        error:(err)=>{
...
//  to prev page
  pervPage(): void {
    if(this.activatePageNum > 1){
     ...
     this._Router.navigate([`/movies/${(currentActive -= 1)}`])
     .finally(()=>{
